...
use cozy_chess::*;
use vampirc_uci::*;
use crate::{search::*, odering::KillerMoves};
...
impl UciSettings {
    pub fn new() -> Self {
      UciSettings{depth:6,nodes:500000,move_time:3000,board:Board::default(),stop:false,quit:false,search:Search::new()}
    }
   ...
   pub fn handle_uci_position(&mut self,startpos:bool, fen:Option<UciFen>, moves:Vec<UciMove>){
       if startpos{
         self.new_game();
       }else{
  ...
  pub fn handle_isready(&self){
   ...
   pub fn handle_go_(&mut self){
      let search =  self.search.iter_deepning(&mut self.board, Some(self.depth), None,None,&self.stop);
