...
impl Search {
    ...
    pub fn negamax(
        ...
        if depth == 0 || board_status != GameStatus::Ongoing || time.elapsed().as_millis() > self.end_time || *hault || self.nodes > self.max_nodes || self.depth > self.max_depth {
        ...
        for (mv, _) in moves_list {
            ...
            alpha = alpha.max(self.curr_score);
        ...
        self.tt.insert(board.hash(), (best_score, alpha, beta, best_move));
       

        if self.depth == depth{ 
            self.pv.push(best_move);
            let pv_lines = self.pv.iter().map(|mv| mv.to_string()).collect::<Vec<String>>().join(" ");
            println!("info depth {} score cp {} time {} nodes {} nps {} pv {}",self.depth,best_score,time.elapsed().as_secs_f64() as u64,self.nodes,(self.nodes as f64/time.elapsed().as_secs_f64()) as u64,pv_lines); 
        }
        return (best_score, best_move);
    }

    pub fn iter_deepning(&mut self,board:&mut Board,depth:Option<u64>,nodes:Option<u64>,time:Option<u128>,hault:&bool) -> (i64,Move){
        let start_time = Instant::now();
        let color = board.side_to_move();
        self.max_nodes = if nodes.is_some(){nodes.unwrap()}else{u64::MAX};
        self.max_depth =  if depth.is_some(){depth.unwrap()}else{100};
        self.end_time =  if time.is_some(){time.unwrap()}else{u128::MAX};
        self.nodes = 1;
        self.depth = 1;

        let mut best_move = DEFAULT_MOVE;
        let mut best_score = if color == Color::Black{INFINITY}else{-INFINITY};


      for d in 1..self.max_depth{
     //   if self.depth < self.max_depth || self.nodes < self.max_nodes || start_time.elapsed().as_millis() < self.end_time || *hault{break};
